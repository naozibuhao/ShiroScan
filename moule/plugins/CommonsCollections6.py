...
@Idea.plugin_register('Class7:CommonsCollections6')
class CommonsCollections6(object):
    ...
    def generator(self, String, fp=JAR_FILE):
        ...
        try:
            ...
            if(r.status_code==200):
                print("[+]   CommonsCollections6模块   key: {} 已成功发送！  状态码:{}".format(str(key),str(r.status_code)))
            else:
                print("[-]   CommonsCollections6模块   key: {} 发送异常！\n[-]   状态码:{}".format(str(key),str(r.status_code)))
